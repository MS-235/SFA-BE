...
//   n_lami                 DOUBLE PRECISION — LAT (optional)
//   n_lgmi                 DOUBLE PRECISION — Long (optional)
...
// ── Helper: peek next A code (no increment) ──────────────────
async function peekNextCode() {
    const seq = await pool.query(
        `SELECT last_value, is_called FROM "Tbl_Area_Mst_n_seq_seq"`
    );
...
async function validateRegion(regionCode) {
    ...
    const result = await pool.query(
        `SELECT "C_Code" FROM "Tbl_Region_Mst" WHERE "C_Code" = $1 LIMIT 1`,
        [regionCode.trim().toUpperCase()]
    );
    if (result.rows.length === 0) {
        throw { status: 400, message: `Region Code "${regionCode}" not found.` };
    }
...
async function create(req, res) {
    const { c_name, c_sh_name, c_region_code, c_sales_hq, c_district_code, c_classification_code, n_lami, n_lgmi } = req.body;
    ...
    try {
        let validatedRegionCode, lat, lng, classification;
        try {
        ...
        const dup = await pool.query(
            `SELECT "C_Code" FROM "Tbl_Area_Mst"
             WHERE LOWER("C_Name") = LOWER($1) AND "C_Region_Code" = $2`,
            [c_name.trim(), validatedRegionCode]
        );
        if (dup.rows.length > 0) {
            return res.status(409).json({ success: false, message: `Area "${c_name}" already exists in this Region.` });
        }

        const inserted = await pool.query(
            `INSERT INTO "Tbl_Area_Mst"
               ("C_Code", "C_Name", "C_Sh_Name", "C_Region_Code", "C_SalesHQ",
                "c_district_code", "C_Classification_Code", "n_lami", "n_lgmi", "d_created", "c_modifier")
             VALUES ('TEMP', $1, $2, $3, $4, $5, $6, $7, $8, NOW(), $9)
             RETURNING "n_seq"`,
            [c_name.trim(), c_sh_name ? c_sh_name.trim() : null, validatedRegionCode,
             c_sales_hq ? c_sales_hq.trim() : null, c_district_code ? c_district_code.trim() : null,
             classification, lat, lng, req.admin.c_user_id]
        );
        const n_seq = parseInt(inserted.rows[0].n_seq, 10);
        const c_code = 'A' + String(n_seq).padStart(5, '0');
        await pool.query(`UPDATE "Tbl_Area_Mst" SET "C_Code" = $1 WHERE "n_seq" = $2`, [c_code, n_seq]);
        ...
        return res.status(201).json({
            ...
            data: {
                ...
                c_classification_code: classification, n_lami: lat, n_lgmi: lng
            }
        });
...
async function getAll(req, res) {
    ...
    try {
        ...
        if (region && region !== '*') {
            params.push(region.includes('*') ? region.replace(/\*/g, '%') : `%${region}%`);
...
async function update(req, res) {
    ...
    try {
        const exists = await pool.query(
            `SELECT "C_Code" FROM "Tbl_Area_Mst" WHERE "C_Code" = $1`, [code.toUpperCase()]
        );
        ...
        const dup = await pool.query(
            `SELECT "C_Code" FROM "Tbl_Area_Mst"
             WHERE LOWER("C_Name") = LOWER($1) AND "C_Region_Code" = $2 AND "C_Code" != $3`,
            [c_name.trim(), validatedRegionCode, code.toUpperCase()]
        );
        if (dup.rows.length > 0) {
            return res.status(409).json({ success: false, message: `Another area with name "${c_name}" already exists in this Region.` });
        }
        ...
        await pool.query(
            `UPDATE "Tbl_Area_Mst"
             SET "C_Name" = $1, "C_Sh_Name" = $2, "C_Region_Code" = $3,
                 ...
                 "n_lami" = $7, "n_lgmi" = $8, "d_modified" = NOW(), "c_modifier" = $9
             WHERE "C_Code" = $10`,
            [c_name.trim(), c_sh_name ? c_sh_name.trim() : null, validatedRegionCode,
             c_sales_hq ? c_sales_hq.trim() : null, c_district_code ? c_district_code.trim() : null,
             classification, lat, lng, req.admin.c_user_id, code.toUpperCase()]
        );
...
async function remove(req, res) {
    ...
    try {
        ...
        await pool.query(`DELETE FROM "Tbl_Area_Mst" WHERE "C_Code" = $1`, [code.toUpperCase()]);

        return res.status(200).json({ success: true, message: `Area "${code}" deleted successfully.` });
