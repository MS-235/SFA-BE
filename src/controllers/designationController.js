...
// ─── GET ALL / SEARCH ─────────────────────────────────────────
// GET /api/masters/designation?name=*          → all active
// GET /api/masters/designation?name=Sales      → search by name
// GET /api/masters/designation?name=S*         → starts with S
async function getAll(req, res) {
    ...
    try {
        ...
        if (!name || name === '*') {
            ...
            query = `
                SELECT "c_code", "c_name", "c_sh_name", "d_created", "d_modified"
                FROM "tbl_desig_mst"
                WHERE "n_deleted" = 0
                ORDER BY "c_code"
            `;
            ...
            // Wildcard: "Sales*" → starts with, "*Sales" → ends with, "*Sales*" → contains
            // Plain "Sales" → exact match (case-insensitive)
...
// ─── GET ONE ──────────────────────────────────────────────────
// GET /api/masters/designation/:code
async function getOne(req, res) {
    const { code } = req.params;

    try {
        const result = await pool.query(
            `SELECT "c_code", "c_name", "c_sh_name", "d_created", "d_modified", "c_modifier"
             FROM "tbl_desig_mst"
        ...
        console.error('Designation getOne error:', err.message);
        return res.status(500).json({ success: false, message: 'Server error.' });
    }
}
